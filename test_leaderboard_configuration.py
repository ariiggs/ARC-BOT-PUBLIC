...
class LeaderboardConfigurationTests(unittest.TestCase):
    ...
    def test_text_color_view_and_overrides_follow_the_active_profile(self):
    ...
    def test_v28_custom_text_color_migrates_to_the_current_profile(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(restored.payload()["version"], 30)
    ...
    def test_setres_uses_dashboard_and_leaderboard_submenus(self):
        ...
        self.assertIn("Text Color", fields)
        self.assertEqual(
...
class LeaderboardBackgroundResetTests(unittest.IsolatedAsyncioTestCase):
    def test_legacy_background_migrates_to_only_the_active_profile(self):
        scrim = SimpleNamespace(
            id="c" * 16,
            leaderboard_orientation="horizontal",
            leaderboard_team_count=16,
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            legacy_path = background_dir / f"{scrim.id}.png"
            ...
            self.assertEqual(current_path.read_bytes(), b"legacy image")
            self.assertEqual(metadata["attachment_url"], legacy_url)
            self.assertFalse(legacy_path.exists())
            self.assertFalse(
                (background_dir / f"{scrim.id}-horizontal-18.png").exists()
            )
    ...
    def test_restore_default_button_tracks_custom_background_presence(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("main.LEADERBOARD_BACKGROUND_UPLOAD_DIR", background_dir):
                ...
                (
                    background_dir / f"{scrim_id}-vertical-24.png"
    ...
    async def test_restore_default_removes_custom_image_and_replaces_preview(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            other_profile_path = background_dir / f"{scrim.id}-horizontal-24.png"
            other_profile_path.write_bytes(b"horizontal custom background")
            ...
            with (
                ...
                other_current_path = _current_leaderboard_background_path(scrim)
            ...
            self.assertTrue(other_profile_path.exists())
